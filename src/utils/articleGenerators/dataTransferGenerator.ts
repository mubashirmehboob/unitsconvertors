...
export function generateDataTransferContent(
  ...
  const introParagraphs = [
    `In modern telecommunications, computer networking, and storage engineering, calculating accurate data transfer rates is essential for evaluating network bandwidth, cloud synchronization throughput, and storage interface performance. Translating **${fromPlural}** (${fromSym}) to **${toPlural}** (${toSym}) bridges the critical boundary between hardware transmission metrics and software data processing rates.`,
    `A common point of confusion in data rate calculations is the fundamental distinction between bit-oriented measurements and byte-oriented measurements. Telecommunications standards, internet service providers (ISPs), Wi-Fi protocols, and optical fiber interfaces universally express line rates in **bits per second** (using a lowercase 'b', such as ${fromSym.includes("b") ? fromSym : "bps, kbps, Mbps, Gbps"}). Conversely, operating systems, file download managers, and storage buses measure transferred payload volume in **bytes per second** (using an uppercase 'B', such as ${toSym.includes("B") ? toSym : "B/s, KB/s, MB/s, GB/s"}), where exactly 1 byte equals 8 bits.`,
