...
import { ArticleGeneratorContext } from "./types";

// Helper standards map for universal fallback lookup
export const universalDisciplineStandardsMap: Record<string, Array<{ organization: string; code: string; title: string }>> = {
  "electrical-calc": [
...
// Helper industry list map for universal fallback lookup
export const universalIndustryListMap: Record<string, string[]> = {
...
/**
 * Universal Fallback Article Generator
 * Used when no specific category template is registered for a discipline.
 */
export function generateUniversalFallbackArticle(ctx: ArticleGeneratorContext): EngineeringArticleData {
  const {
    tool,
    discipline,
    canonicalUrl,
  ...
  const activeIndustries = universalIndustryListMap[normalizedDiscId] || [
  ...
  const bestPractices: string[] = [
    ...
    `Incorporate standard engineering design guidelines recommended by governing bodies (${universalDisciplineStandardsMap[normalizedDiscId]?.[0]?.organization || "ISO"}).`,
    `Use normalized SI base units during intermediate steps to eliminate conversion errors.`
  ];

  const standardsReferences = universalDisciplineStandardsMap[normalizedDiscId] || universalDisciplineStandardsMap["general-engineering-calc"];
  ...
  const faqs: EngineeringArticleFAQ[] = [
    ...
    },
    {
      ...
      answer: `Calculations in ${discipline.name} align with reference codes established by standards organizations such as ${standardsReferences.map(s => s.organization).join(", ")}.`
    }
  ];
